...
MU_TEST(test_left_border) {
    int x = 52;   // within 2px of left border
    int y = 100;  // inside vertical bounds
    mu_assert(UiUtils_CoordinateIsOnBorderOf(x, y, borderWidth, testPos) == LEFT,
              "Should be left Border");
}

MU_TEST(test_right_border) {
    int x = 198;  // within 2px of right border
    int y = 100;
    mu_assert(UiUtils_CoordinateIsOnBorderOf(x, y, borderWidth, testPos) == RIGHT,
              "Should be right Border");
}

MU_TEST(test_top_border) {
    int x = 150;
    int y = 102;  // within 2px of top border
    mu_assert(UiUtils_CoordinateIsOnBorderOf(x, y, borderWidth, testPos) == TOP,
              "Should be top Border");
}

MU_TEST(test_bottom_border) {
    int x = 150;
    int y = 148;  // within 2px of bottom border
    mu_assert(UiUtils_CoordinateIsOnBorderOf(x, y, borderWidth, testPos) == BOTTOM,
              "Should be bottom Border");
}

MU_TEST(test_outside_border) {
    int x = 300;
    int y = 300;
    mu_assert(UiUtils_CoordinateIsOnBorderOf(x, y, borderWidth, testPos) == BORDER_NONE,
              "Should be no Border");
}
