...
void UiUtils_DrawColoredRectangle(AbsolutePos_t pos, COLORREF colorBG, COLORREF colorBorder,
                                  int BorderSize) {
...
void UiUtils_DrawTextTheme(AbsolutePos_t pos, const char* name, UINT format, HFONT font,
                           COLORREF color) {
...
HFONT UiUtils_CreateFont(void) {
    return CreateFont(-MulDiv(10, GetDeviceCaps(currentWindowState.memHDC, LOGPIXELSY), 72), 0, 0,
                      0, FW_NORMAL, FALSE, FALSE, FALSE, DEFAULT_CHARSET, OUT_DEFAULT_PRECIS,
                      CLIP_DEFAULT_PRECIS, CLEARTYPE_QUALITY, DEFAULT_PITCH | FF_DONTCARE,
                      TEXT("Segoe UI"));
}

UiUtils_BorderEnum UiUtils_CoordinateIsOnBorderOf(int x, int y, int borderWidth,
                                                  AbsolutePos_t pos) {
...
AbsolutePos_t getPosToContainerAbsolute(const AbsolutePos_t* parentAbsPos,
                                        AbsolutePos_t widgetAbsPos) {
    AbsolutePos_t absPos;
...
bool UiUtils_WidgetFitsInContainer(AbsolutePos_t widget, AbsolutePos_t container) {
    return widget.left >= container.left && widget.top >= container.top &&
           widget.right <= container.right && widget.bottom <= container.bottom;
}
...
HFONT getFontForRect(HDC hdc, char text[], RECT box) {
    ...
    while (minSize <= maxSize) {
        ...
        hFont = CreateFontA(-fontSize, 0, 0, 0, FW_NORMAL, FALSE, FALSE, FALSE, DEFAULT_CHARSET,
    ...
    hFont = CreateFontA(-maxSize, 0, 0, 0, FW_NORMAL, FALSE, FALSE, FALSE, DEFAULT_CHARSET,
                        OUT_DEFAULT_PRECIS, CLIP_DEFAULT_PRECIS, DEFAULT_QUALITY,
                        DEFAULT_PITCH | FF_DONTCARE, "Segoe UI");
...
void UiUtils_draw3SliceHelper(AbsolutePos_t pos, HICON left, HICON center, HICON right) {
    ...
    if (canDraw) {
        for (int i = 0; i < fullTiles; ++i) {
            ...
            DrawIconEx(currentWindowState.memHDC, x, pos.top, center, height, height, 0, NULL,
                       DI_NORMAL);
        }
    }
    ...
    if (canDraw && remainder > 0) {
        ...
        AlphaBlend(currentWindowState.memHDC, centerStartX + fullTiles * height, pos.top, remainder,
                   height, iconDC, 0, 0, remainder, height, blendFunc);
    ...
    if (canDraw) {
        DrawIconEx(currentWindowState.memHDC, pos.left, pos.top, left, height, height, 0, NULL,
                   DI_NORMAL);
        DrawIconEx(currentWindowState.memHDC, pos.right - height, pos.top, right, height, height, 0,
                   NULL, DI_NORMAL);
    }
...
static void drawPartialTile(HDC targetHDC, int x, int y, int width, int height, HICON icon,
...
static void drawHorizontalEdge(HDC hdc, int startX, int y, int centerWidth, HICON edgeIcon,
...
static void drawVerticalEdge(HDC hdc, int x, int startY, int centerHeight, HICON edgeIcon,
                             int sliceWidth, int sliceHeight) {
...
static void drawCenterArea(HDC hdc, int startX, int startY, int centerWidth, int centerHeight,
                           HICON centerIcon, int sliceWidth, int sliceHeight) {
    ...
    if (remainderX > 0) {
        for (int y = 0; y < tilesY; y++) {
            ...
            int drawY = startY + y * sliceHeight;
            drawPartialTile(hdc, drawX, drawY, remainderX, sliceHeight, centerIcon, sliceWidth,
                            sliceHeight);
        }
    }
    ...
    if (remainderY > 0) {
        for (int x = 0; x < tilesX; x++) {
            ...
            drawPartialTile(hdc, drawX, drawY, sliceWidth, remainderY, centerIcon, sliceWidth,
                            sliceHeight);
        }
    }
    ...
    if (remainderX > 0 && remainderY > 0) {
        int drawX = startX + tilesX * sliceWidth;
        int drawY = startY + tilesY * sliceHeight;
        drawPartialTile(hdc, drawX, drawY, remainderX, remainderY, centerIcon, sliceWidth,
                        sliceHeight);
    }
}

void draw9SliceHelper(AbsolutePos_t pos, HICON topLeft, HICON topCenter, HICON topRight,
                      HICON midLeft, HICON center, HICON midRight, HICON bottomLeft,
                      HICON bottomCenter, HICON bottomRight, int sliceWidth, int sliceHeight) {
    int width = pos.right - pos.left;
    int height = pos.bottom - pos.top;
    ...
    DrawIconEx(hdc, pos.right - sliceWidth, pos.top, topRight, sliceWidth, sliceHeight, 0, NULL,
               DI_NORMAL);
    DrawIconEx(hdc, pos.left, pos.bottom - sliceHeight, bottomLeft, sliceWidth, sliceHeight, 0,
               NULL, DI_NORMAL);
    DrawIconEx(hdc, pos.right - sliceWidth, pos.bottom - sliceHeight, bottomRight, sliceWidth,
               sliceHeight, 0, NULL, DI_NORMAL);

    // Draw edges
    drawHorizontalEdge(hdc, pos.left + sliceWidth, pos.top, centerWidth, topCenter, sliceWidth,
                       sliceHeight);
    drawHorizontalEdge(hdc, pos.left + sliceWidth, pos.bottom - sliceHeight, centerWidth,
                       bottomCenter, sliceWidth, sliceHeight);
    drawVerticalEdge(hdc, pos.left, pos.top + sliceHeight, centerHeight, midLeft, sliceWidth,
                     sliceHeight);
    drawVerticalEdge(hdc, pos.right - sliceWidth, pos.top + sliceHeight, centerHeight, midRight,
                     sliceWidth, sliceHeight);

    // Draw center
    drawCenterArea(hdc, pos.left + sliceWidth, pos.top + sliceHeight, centerWidth, centerHeight,
                   center, sliceWidth, sliceHeight);
}

void UiUtils_DrawHoverOverlay(AbsolutePos_t pos) {
    HDC overlayDC = CreateCompatibleDC(currentWindowState.memHDC);
    HBITMAP overlayBitmap = CreateCompatibleBitmap(currentWindowState.memHDC, pos.right - pos.left,
                                                   pos.bottom - pos.top);
    ...
    AlphaBlend(currentWindowState.memHDC, pos.left, pos.top, overlayRect.right, overlayRect.bottom,
               overlayDC, 0, 0, overlayRect.right, overlayRect.bottom, blend);
