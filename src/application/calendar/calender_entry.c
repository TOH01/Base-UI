...
void renderCalendarEntries(container_t* grid, calender_entry_t* entries, int numEntries,
    ...
    for (int i = 0; i < numEntries; i++) {
        ...
        if (entry->type != ENTRY_TYPE_DELETED) {
            ...
            switch (entry->type) {
                ...
                case ENTRY_CHECKBOX: {
                    checkboxWidget_t* checkbox =
                        customCheckbox_initCheckbox((CommonPos_t){0, 0, 0, 0}, &entry->data.state);
                    checkbox->onStateChange = onDataChangeCallback;
                    widget = (BaseWidget_t*)checkbox;
                } break;
                case ENTRY_NUM: {
                    inputWidget_t* input =
                        customInput_initInputNumeric((CommonPos_t){0, 0, 0, 0}, &entry->data.count);
            ...
            buttonWidget_t* label = customButton_initButton((CommonPos_t){0, 0, 0, 0},
                                                            labelClickCallback, labelIdCounter);
            labelIdCounter++;
            ...
            // Move to the next entry (2 rows for the entry + 1 row spacer already counted at start
            // of next loop)
            currentRow += 2;
        } else {
