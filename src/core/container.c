        ...
        for (int c = 0; c < cols; ++c) {
            ...
            for (int rr = r; rr < r + spanRows; ++rr) {
                for (int cc = c; cc < c + spanCols; ++cc) {
                    int v = rr * cols + cc;
                    if (v >= 0 && v < (int)cells) visited[v] = 1;
                }
            }
        }
    }
...
container_t* Container_initContainer(containerPos_t pos) {
...
container_t* initGridContainer(containerPos_t pos, int rows, int cols) {
    container_t* gridContainer = Container_initContainer(pos);
...
container_t* windowAddContainer(AbsolutePos_t pos) {
    container_t* container = Container_initContainer(pos);
