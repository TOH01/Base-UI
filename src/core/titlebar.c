...
void drawTitlebar(HDC hdc) {
    ...
    if (active) {
        ...
        switch (active) {
            ...
            case MINIMIZE:
                ...
                break;
            default:
                hoverColor = RGB(255, 255, 255);  // fallback color
        }
    ...
    Rectangle(currentWindowState.memHDC, restoreRect.left + widthRestore * 0.2, restoreRect.top + heightRestore * 0.2,
              restoreRect.right - widthRestore * 0.2, restoreRect.bottom - heightRestore * 0.2);
...
void initTitlebar(void) {
    WmParamTable_Insert(currentWindowState.handlerTable, WM_MOUSEMOVE, &Titlebar_WmMouseMove);
    WmParamTable_Insert(currentWindowState.handlerTable, WM_NCMOUSELEAVE, &Titlebar_MouseLeaveCallback);
}
