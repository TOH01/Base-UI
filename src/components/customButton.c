...
static void draw3SliceButton(BaseWidget_t *baseWidget) {
	...
	draw3SliceHelper(pos, button->images[0], button->images[1], button->images[2]);
	...
	if (button->theme != NULL) {
		...
		if (UiUtils_TextFitsBoxTheme(button->name, pos, button->theme->text.font)) {
			if (button->beingPressed) {
				...
				UiUtils_DrawTextTheme(pos, button->name, button->theme->text.formatFlags, smallerFont, button->theme->text.color);

				DeleteObject(smallerFont);
			} else {
				UiUtils_DrawTextTheme(pos, button->name, button->theme->text.formatFlags, button->theme->text.font, button->theme->text.color);
			}
		}
	}
#ifdef DEBUG
...
static void onHoverEndButton(BaseWidget_t *base) {
	...
	button->beingPressed = false; // fix error, that when leaving button area while holding left click and then releasing would leave beingPressed true
	InvalidateRect(currentWindowState.hwnd, NULL, FALSE);
}
