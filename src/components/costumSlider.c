...
sliderWidget_t* customSlider_initSlider(CommonPos_t pos, int* value, int range) {
    ...
    if (!sliderHandlersRegistered) {
        WmParamTable_Insert(currentWindowState.handlerTable, WM_MOUSEMOVE, (void*)&mouseMoveSlider);
        WmParamTable_Insert(currentWindowState.handlerTable, WM_LBUTTONUP, (void*)&mouseReleaseSlider);
