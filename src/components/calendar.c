...
static void drawCalendar(BaseWidget_t *base) {
	...
	COLORREF selectedEndColor = RGB(255, 210, 170);
	...
	for (int row = 0; row < rows; row++) {
		for (int col = 0; col < cols; col++) {
			...
			if (dayNum > 0) {
				if (dayNum == cal->todayDay && cal->month == cal->todayMonth && cal->year == cal->todayYear) {
					fill = todayColor;
				}
				if (dayNum == cal->selectedDay) {
					fill = selectedColor;
				}
				if (dayNum == cal->selectedEndDay) {
					fill = selectedEndColor;
				}
			}
...
static void onCalendarClick(BaseWidget_t *base, int x, int y, ClickType_t clickType) {
	...
	if (dayNum > 0 && dayNum <= cal->daysInMonth && clickType == CLICK_TYPE_LUP) {
	...
	} else if (dayNum > 0 && dayNum <= cal->daysInMonth && clickType == CLICK_TYPE_RUP) {

		if (dayNum == cal->selectedEndDay) {
			cal->selectedEndDay = 0;
		} else {
			cal->selectedEndDay = dayNum;
		}

		if (cal->dateSelectCallback != NULL) {
			cal->dateSelectCallback();
		}
