...
void drawTextDump(BaseWidget_t* baseWidget) {
    ...
    if (currentLine == NULL) {
        ...
        UiUtils_DrawTextTheme(text, "EMPTY TEXT DUMP ERROR", textDump->theme->text.formatFlags, textDump->theme->text.font,
                              textDump->theme->text.color);
    } else {
        AbsolutePos_t text = baseWidget->pos;
        text.bottom = text.top + lineHeight;

        for (int i = 0; (i < visibleLines) && (currentLine != NULL); i++) {
            UiUtils_DrawTextTheme(text, currentLine->line, textDump->theme->text.formatFlags, textDump->theme->text.font,
                                  textDump->theme->text.color);
            text.top += lineHeight;
...
textDumpWidget_t* customTextDump_initTextDump(CommonPos_t pos) {
    ...
    if (!scrollCallbackRegisters) {
        WmParamTable_Insert(currentWindowState.handlerTable, WM_MOUSEWHEEL, &scrollCallback);
