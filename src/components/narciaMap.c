...
bool screenPosToNarciaPos(narciaMap_t* map, int screenX, int screenY, Coordinate_t* outTile) {
    ...
    if (tileX < 0 || tileX >= map->mapSize || tileY < 0 || tileY >= map->mapSize) return false;
...
static void drawNarciaMap(BaseWidget_t* base) {
    ...
    if (map->paths) {
        ...
        for (int i = 0; i < pathCount; i++) {
            ...
            if (!path || !path->tiles || path->tileCount < 2) continue;
...
LRESULT mouseMoveNarciaMap(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam) {
    ...
    if (activeNaricMap == NULL) return 0;
    ...
    // Clamp to map bounds
    if (newMiddleX < 0) newMiddleX = 0;
    if (newMiddleY < 0) newMiddleY = 0;
    if (newMiddleX >= activeNaricMap->mapSize) newMiddleX = activeNaricMap->mapSize - 1;
    if (newMiddleY >= activeNaricMap->mapSize) newMiddleY = activeNaricMap->mapSize - 1;
...
#define GRAPH(i, j) graph[(i) * townCount + (j)]
    ...
    for (int i = 0; i < townCount; i++) {
        ...
        if (t->x == start.x && t->y == start.y) startIndex = i;
        if (t->x == end.x && t->y == end.y) endIndex = i;
    }
    ...
    int pathSize = 0;
    for (int i = endIndex; i != -1; i = nodes[i].previousIndex) pathSize++;

    Coordinate_t* tileList = malloc(sizeof(Coordinate_t) * pathSize);
    int index = pathSize - 1;
    for (int i = endIndex; i != -1; i = nodes[i].previousIndex) tileList[index--] = nodes[i].coord;
